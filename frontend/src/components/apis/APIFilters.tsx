...
/**
 * API filter controls — category, pricing, auth, sort.
...
export function APIFilters({ totalResults, className = "" }: APIFiltersProps) {
  const {
    filters,
    setCategory,
    setPricing,
  ...
  return (
    <div className={className}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-controls="api-filter-panel"
          >
      ...
      >
        ...
        {/* Pricing + Auth — side by side on wider screens */}
            ...
            <div className="flex flex-wrap gap-1.5">
              {PRICING_OPTIONS.map((opt) => (
                <FilterChip
                  ...
                  onClick={() =>
                    setPricing(filters.pricing === opt.value ? "" : opt.value)
                  }
                />
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {AUTH_OPTIONS.map((opt) => (
                <FilterChip
                  key={opt.value}
                  label={opt.label}
                  active={filters.auth === opt.value}
                  onClick={() =>
                    setAuth(filters.auth === opt.value ? "" : opt.value)
                  }
                />
              ))}
            </div>
